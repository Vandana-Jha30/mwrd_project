...
def train(args):
    ...
    perceptual = PerceptualLoss(layer='relu3_3').to(device)
    def criterion(pred, target):
        return 0.6 * l1(pred, target) + 0.6 * charbonnier(pred, target) + 0.5 * ssim_loss(pred, target) + 0.3 * perceptual(pred, target)

    optimizer = optim.AdamW(model.parameters(), lr=args.lr, weight_decay=1e-4, betas=(0.9, 0.999))
    scheduler = torch.optim.lr_scheduler.CosineAnnealingWarmRestarts(optimizer, T_0=10, T_mult=3, eta_min=1e-6)
